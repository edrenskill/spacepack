...
if (! function_exists('currency')) {
    function currency($price = "") {
        ...
        if ($price != "") {
            ...
            $CI->db->where('symbol', $currency_code);
...
{
    // Checks if a video is youtube, vimeo or any other
    function ellipsis($long_string, $max_character = 28) {
        $short_string = strlen($long_string) > $max_character ? substr($long_string, 0, $max_character)."..." : $long_string;
        return $short_string;
    }
}

if ( ! function_exists('desc_ellipsis'))
{
    // Checks if a video is youtube, vimeo or any other
    function desc_ellipsis($long_string, $max_character = 49) {
        $short_string = strlen($long_string) > $max_character ? substr($long_string, 0, $max_character)."..." : $long_string;
        return $short_string;
    }
}

if ( ! function_exists('summary_ellipsis'))
{
    // Checks if a video is youtube, vimeo or any other
    function summary_ellipsis($long_string, $max_character = 80) {
        $short_string = strlen($long_string) > $max_character ? substr($long_string, 0, $max_character)."..." : $long_string;
        return $short_string;
    }
}
